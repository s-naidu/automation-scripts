...
def send_message(cursor):
    """
     Send message to slack if any discrepancy is found
    """
    msg = ''
    for row in cursor:
        msg = 'Info : {encoding_order_detail_total : ' + str(row['EO_UPCS_COUNT']) + ', encoding_queue_detail_total : ' + str(row['EQ_UPCS_COUNT']) + '}' 
        slack_msg = {
            ...
            'attachments' : [
                {
                    ...
                    'color' : '#DC143C', 
                    'title' : 'Discrepencies in encoding order ID - {}'.format(row['ENCODING_ORDER_ID']), 
                    'text' : msg
                }
            ]    
        }
